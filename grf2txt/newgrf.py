...
class NewGRF:
    ...
    grf_version: Optional[int]  # NewGRF spec version
    version: Optional[int]  # Version of NewGRF
    min_compatible_version: Optional[int]
    container_version: Optional[int]

    plurals: dict[int, int]
    genders: dict[int, dict[int, list[str]]]
    cases: dict[int, dict[int, list[str]]]
    strings: dict[str, GRFString]
    ...
    def read_pseudo(self, sprite_index: int, pseudo: bytes, state: ParserState) -> int:
        ...
        if action == 0x00:
                        ...
                        if prop == 0x08:
                            ...
                            for i in range(num_ids):
                                while True:
                                    ...
                                    if gi == 0:
                                        break
                                    self.genders.setdefault(first_id + i, dict()).setdefault(gi, []).append(
                                        reader.str().decode("utf-8")
                                    )
                        elif prop == 0x14:
                            for i in range(num_ids):
                                while True:
                                    ...
                                    if gi == 0:
                                        break
                                    self.cases.setdefault(first_id + i, dict()).setdefault(gi, []).append(
                                        reader.str().decode("utf-8")
                                    )
                        elif prop == 0x15:
                            for i in range(num_ids):
            ...
            if lang_id < 0x80:
                ...
                lang_id -= 0x80
            for i in range(num_ids):
                self.strings.setdefault(str_id(first_id + i), GRFString()).translations[lang_id] = reader.str()
        elif action == 0x05:
            reader.skip(1)
            ...
            self.strings.setdefault(self.STR_GRF_NAME, GRFString()).translations[0x7F] = reader.str()
            self.strings.setdefault(self.STR_GRF_DESCRIPTION, GRFString()).translations[0x7F] = reader.str()
        elif action == 0x0A:
            num_sets = reader.uint8()
            ...
            if state.actionb_string is None:
                state.actionb_string = GRFString()
                self.strings[self.str_error(sprite_index)] = state.actionb_string
            reader.skip(1)
            lang_id = reader.uint8()
            reader.skip(1)
            text = reader.str()
            if text:
                state.actionb_string.translations[lang_id] = text
            if lang_id == 0x7F:
                state.actionb_string = None
        elif action == 0x11:
        ...
        while True:
            ...
            if type_id == ord("C"):
                ...
                if subpath == b"INFONAME":
                    self.strings.setdefault(self.STR_GRF_NAME, GRFString()).translations[grflangid] = sdata
                elif subpath == b"INFODESC":
                    self.strings.setdefault(self.STR_GRF_DESCRIPTION, GRFString()).translations[grflangid] = sdata
                elif subpath == b"INFOURL_":
                    self.strings.setdefault(self.STR_GRF_URL, GRFString()).translations[grflangid] = sdata
                elif subpath[:8] == b"INFOPARA" and subpath[12:] == b"NAME":
                    param = int.from_bytes(subpath[8:12], byteorder="little", signed=False)
                    self.strings.setdefault(self.str_param_name(param), GRFString()).translations[grflangid] = sdata
                elif subpath[:8] == b"INFOPARA" and subpath[12:] == b"DESC":
                    param = int.from_bytes(subpath[8:12], byteorder="little", signed=False)
                    self.strings.setdefault(self.str_param_description(param), GRFString()).translations[
                        grflangid
                    ] = sdata
            else:
                return False
